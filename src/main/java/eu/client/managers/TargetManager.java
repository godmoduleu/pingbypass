...
public class TargetManager implements IMinecraft {
    ...
    public void onTick(TickEvent event) {
        ...
        PlayerEntity caTarget = Pingbypass.MODULE_MANAGER.getModule(AutoCrystalModule.class).getTarget();

        synchronized (targets) {
            targets.removeIf(t -> System.currentTimeMillis() - t.time > 15000); // Remove targets if 15 seconds since
                                                                                // last time they were targeted has
                                                                                // passed

            if (caTarget != null)
                targets.add(new Target(caTarget));
        }
    }

    @SubscribeEvent
    public void onPlayerDeath(PlayerDeathEvent event) {
        if (mc.player == null || mc.world == null || !isTarget(event.getPlayer()))
            return;

        synchronized (targets) {
            Pingbypass.EVENT_HANDLER.post(new TargetDeathEvent(event.getPlayer()));
