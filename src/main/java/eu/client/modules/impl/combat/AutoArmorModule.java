...
public class AutoArmorModule extends Module {
    public ModeSetting health = new ModeSetting("Health", "The health priority to apply.", "Highest",
            new String[] { "Highest", "Lowest", "Any" });
    public BooleanSetting elytraPriority = new BooleanSetting("ElytraPriority", "Prioritizes elytra over armor pieces.",
            true);
    public BooleanSetting preserve = new BooleanSetting("Preserve",
            "Preserve low health armor to avoid it from breaking.", false);
    public NumberSetting preserveHealth = new NumberSetting("PreserveHealth",
            "The minimum health of armor to preserve it.", 20.0f, 10.0f, 50.0f);
    public BooleanSetting elytra = new BooleanSetting("Elytra", "Equips an elytra instead of a chestplate.", false);
    public BooleanSetting smartElytra = new BooleanSetting("SmartElytra",
            "Chooses when to enable elytra in a more convenient way.", false);
    ...
    public void onKeyInput(KeyInputEvent event) {
        if (shouldRunOnProxy())
            return;
        if (getNull() || !smartElytra.getValue())
            return;
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if (smartElytra.getValue() && elytra.getValue()) {
            if ((mc.player.isOnGround() && !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem)
                    || EntityUtils.isInWeb(mc.player)))
        ...
        if (ticks <= 0) {
            ...
            if (!(elytraPriority.getValue() && mc.player.getInventory().getStack(38).getItem() == Items.ELYTRA)
                    || elytra.getValue())
    ...
    private void update(EquipmentSlot type, int x) {
        ...
        int slot = type == EquipmentSlot.HEAD ? 39
                : type == EquipmentSlot.CHEST ? 38 : type == EquipmentSlot.LEGS ? 37 : 36;
        int armor = flag ? elytraSlot : findArmor(type);
        int best = flag ? compareElytra(38, armor) : compare(slot, armor, true);

        if (armor != -1 && best != slot) {
            mc.interactionManager.clickSlot(mc.player.playerScreenHandler.syncId, x, 0, SlotActionType.PICKUP,
                    mc.player);
            mc.interactionManager.clickSlot(mc.player.playerScreenHandler.syncId, InventoryUtils.indexToSlot(armor), 0,
    ...
    private int compare(int x, int y, boolean swap) {
        ...
        if (DamageUtils.getProtectionAmount(mc.player.getInventory().getStack(x)) < DamageUtils
                .getProtectionAmount(mc.player.getInventory().getStack(y)))
            return y;
        ...
        if (!swap) {
            if (health.getValue().equals("Highest") && getDurability(x) < getDurability(y))
                return y;
            else if (health.getValue().equals("Lowest") && getDurability(x) > getDurability(y))
                return y;
        }
    ...
    private EquipmentSlot getSlotType(ItemStack itemStack) {
        if (itemStack.contains(DataComponentTypes.GLIDER))
