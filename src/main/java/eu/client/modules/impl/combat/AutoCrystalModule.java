...
public class AutoCrystalModule extends Module {
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) EUClient.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)));
        if (rotate.getValue().equalsIgnoreCase("Normal")) EUClient.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)), EUClient.ROTATION_MANAGER.getModulePriority(this));
    ...
    public void onDestroyBlock(DestroyBlockEvent event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) EUClient.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), this, EUClient.ROTATION_MANAGER.getModulePriority(this) + 1);
        if (!rotate.getValue().equalsIgnoreCase("None")) EUClient.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
    ...
    private void placeCrystals(boolean sequential) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) EUClient.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), EUClient.ROTATION_MANAGER.getModulePriority(this));
        ...
        placeRunnable = () -> {
            ...
            if (rotate.getValue().equalsIgnoreCase("Packet")) EUClient.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
            ...
            if (godSync.getValue()) {
                boolean flag = !antiKick.getValue() || !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem) && !(mc.player.getOffHandStack().getItem() instanceof ExperienceBottleItem) && !EUClient.MODULE_MANAGER.getModule(ThrowXPModule.class).isToggled();
    ...
    private void place(BlockPos position) {
        ...
        eu.client.utils.minecraft.NetworkUtils.sendSequencedPacket(sequence ->
                new PlayerInteractBlockC2SPacket(hand, new BlockHitResult(WorldUtils.getHitVector(position, direction), direction, position, false), sequence));
