...
public class WorldUtils implements IMinecraft {
    ...
    public static void placeBlock(BlockPos position, Direction direction, Hand hand, Runnable runnable, boolean rotate, boolean crystalDestruction, boolean render) {
        ...
        if (rotate) EUClient.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(vec3d.getX(), vec3d.getY(), vec3d.getZ()));
        ...
        if (!isProxy && sprint) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.STOP_SPRINTING));
        ...
        if (!isProxy && sprint) mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(mc.player, ClientCommandC2SPacket.Mode.START_SPRINTING));
